...
(function main() {
    ...
    function onSelectionChanged(tries = 0) {
        ...
        try {
            ...
            if (!venueIsEditabe(venue)) return;

            // Exclude gas station and EVCS categories (don't ever want to delete those by splitting):
            // SDK: FR submitted to add venue attributes
            const OLD_VENUE = W.model.venues.getObjectById(venue.id);
            if (OLD_VENUE.attributes.categories.some(cat => ['GAS_STATION', 'CHARGING_STATION'].includes(cat))) return;
    ...
    function venueIsEditabe(venue) {
        ...
        return venue?.arePropertiesEditable() || venue.arePropertiesSuggestible();
    }
    ...
    function onSplitPoiButtonClick() {
        ...
        if (OLD_VENUE.attributes.categories.some(cat => ['GAS_STATION', 'CHARGING_STATION'].includes(cat))) {
